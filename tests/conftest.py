...
from framework.core import Event, Pipeline, AbstractProcessor
from framework.builder import AbstractBuilder, builder as builder_decorator
...
class RecordingSink(AbstractProcessor):
    ...
    def built_to(self, to_obj) -> List[Event]:
        return [e for e in self.events if getattr(e, "name", None) == "built" and getattr(e, "to", None) is to_obj]

    def resolves_from(self, sender_obj) -> List[Event]:
        return [e for e in self.events if getattr(e, "name", None) == "resolve" and getattr(e, "sender", None) is sender_obj]

    def first_index(self, predicate: Callable[[Event], bool]) -> Optional[int]:
        for idx, e in enumerate(self.events):
            if predicate(e):
                return idx
        return None

    def indices(self, predicate: Callable[[Event], bool]) -> List[int]:
...
class TriggerAfterReadySink(AbstractProcessor):
    ...
    def process(self, context, event):
        ...
        if (not self.did_trigger
            ...
            context.submit(Event(name="resolve", target=self.target, sender=self))
...
def make_pipeline():
    ...
    def _make(*processors: AbstractProcessor) -> Pipeline:
        return Pipeline(processors=list(processors), strict_interest_inference=False, workspace=None)
    return _make
