    ...
    def test_many_workers_component_count(self):
        ...
        for num_workers in [1, 5, 10, 50]:
            ...
            assert len(components) == expected_count, (
                f"Failed for num_workers={num_workers}"
            )
    ...
    def test_high_worker_count_distribution(self):
        ...
        for i, count in enumerate(counts):
            assert count == expected_per_worker, (
                f"Worker {i} got {count}, expected {expected_per_worker}"
            )


class TestParallelizePositionalArgs:
    """Tests for parallelize() worker_init_args parameter (bug fix regression tests)."""

    def test_parallelize_accepts_positional_args(self):
        """Workers with required positional args can be created via parallelize."""
        from framework.reactive import parallelize
    ...
    def test_parallelize_mixed_positional_and_keyword_args(self):
        """Both positional and keyword args work together in parallelize."""
        from framework.reactive import parallelize
        ...
        wrappers = [c for c in components if isinstance(c, LoadBalancerWorkerWrapper)]
        assert wrappers[0].worker.required == "my_value"
        assert wrappers[0].worker.optional == 20
